"""
NexHost V4 - Flask Backend Application
=======================================
...
def create_app():
    ...
    db_path = os.environ.get(
        'DB_PATH',
        os.path.join(DATA_DIR, 'nexhost.db')
    )
    db = Database(db_path)
    ...
    def check_maintenance():
        ...
        if request.path.startswith('/frontend/') or not request.path.startswith('/api/'):
            return
        ...
        if settings_manager.db.get_setting('maintenance_mode') == 'true':
            ...
            if not user or user['role'] not in ['admin', 'superadmin']:
                return jsonify({
                    'success': False,
                    'maintenance': True,
                    'message': settings_manager.db.get_setting('maintenance_message') or 'النظام تحت الصيانة، سيعود قريباً 🔧',
    ...
    def login():
        ...
        if result['success']:
            ...
            if user_id != 0:
                db.log_activity(user_id, 'login', f"تسجيل دخول: {username}")
    ...
    def get_project_status(project_id):
        ...
        return jsonify({
            'success': True,
            'status': 'running' if is_running else project['status'],
            'pid': project.get('pid'),
            'uptime': process_runner.get_uptime(project_id) if is_running else None
        }), 200
    
    @app.route('/api/projects/<int:project_id>/hot-reload', methods=['POST'])
    def hot_reload(project_id):
        """Hot reload project file"""
        user, error_response, status = require_auth()
        if error_response:
            return error_response, status
        ...
        result = project_manager.hot_reload(
            project_id=project_id,
            user_id=user['id'],
            user_role=user['role'],
            file_path=file_path,
    ...
    def health_check():
        ...
        return jsonify({'success': True, 'status': 'online', 'version': '4.0.0'}), 200
    ...
    # =============================================================================
    # ADMIN ROUTES (Maintenance, Broadcast, Quick Links)
    # =============================================================================
    ...
    def get_active_broadcast():
        """Get active broadcast (auth required)"""
        user, error_response, status = require_auth()
        if error_response:
            return error_response, status
    ...
    def deactivate_broadcast(broadcast_id):
        """Deactivate broadcast (admin only)"""
        admin, error_response, status = require_admin()
        if error_response:
            return error_response, status
...
# Required for: gunicorn backend.app:app
import os as _os
